...
class KrxDailyPriceProvider(BaseProvider[list[KrxDailyPriceItem]]):
    ...
    def connection_status(self) -> ConnectionStatusItem:
        ...
        return ConnectionStatusItem(
            ...
            detail=(
                "인증키가 설정됐으나 일별가격 호출은 아직 검증되지 않았습니다."
                if configured
    ...
    async def fetch(
        self,
        **request: object,
    ) -> ApiResponse[list[KrxDailyPriceItem]]:
        ...
        try:
            ...
            records = TypeAdapter(list[KrxDailyPriceItem]).validate_python(rows)
        ...
        if not records:
            return ApiResponse(
                ...
                error_message="KRX returned an empty daily-price list",
            )
        return ApiResponse(
